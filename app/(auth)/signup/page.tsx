...
const magicSchema = z.object(baseFields);
...
function SignupForm() {
  ...
  const magicForm = useForm<MagicForm>({ resolver: zodResolver(magicSchema), defaultValues: { name: '', email: '', role: defaultRole } });
  const passwordForm = useForm<PasswordForm>({ resolver: zodResolver(passwordSchema), defaultValues: { name: '', email: '', role: defaultRole, password: '', confirm: '' } });
  ...
  const onPasswordSubmit = async (data: PasswordForm) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        credentials: 'include',
        body: JSON.stringify({ email: data.email, password: data.password, name: data.name, role: data.role.toUpperCase() }),
      });
      const result = await res.json();
      if (!res.ok) throw new Error(result.error || 'Registration failed');
      toast.success('Account created!');
      const roleMap: Record<string, string> = { CUSTOMER: '/dashboard/customer', TECHNICIAN: '/dashboard/technician' };
  ...
  return (
    <div className="min-h-screen flex bg-[#0a0a0f]">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] p-12 relative overflow-hidden">
        <div className="absolute w-[400px] h-[400px] rounded-full blur-[120px] opacity-20 bg-[#00ffa3] -top-20 -left-20 pointer-events-none" />
        ...
        <div className="relative z-10 space-y-6">
          <Shimmer className="text-4xl font-extrabold text-white leading-tight" duration={4}>
            Join 2,400+ technicians.
          </Shimmer>
          <p className="text-white/40 text-base leading-relaxed max-w-sm">
            Whether you need a fix or want to earn - Malaysia Co (Maintenance Services) connects you with the right people, fast.
          </p>

          <div className="space-y-3 pt-2">
            {[
              { role: 'Customer', desc: 'Post jobs, get quotes, hire vetted pros', color: '#00d4ff' },
              { role: 'Technician', desc: 'Browse jobs, set your rate, earn flexibly', color: '#00ffa3' },
            ].map(({ role, desc, color }) => (
              <div key={role} className="flex items-start gap-3 p-3 rounded-xl border border-white/8" style={{ background: 'rgba(255,255,255,0.03)' }}>
        ...
        <div className="relative z-10 p-4 rounded-2xl border border-white/8" style={{ background: 'rgba(255,255,255,0.03)' }}>
          <p className="text-sm text-white/50 italic">"Signed up, got my first job in 20 minutes."</p>
          <p className="text-xs text-white/25 mt-1">-- James O., Technician</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative overflow-y-auto">
        ...
        <div className="relative z-10 w-full max-w-md py-8">
          ...
          <div className="rounded-3xl p-8 border border-white/10" style={{ background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(20px)', boxShadow: '0 0 60px rgba(0,255,163,0.06), inset 0 1px 0 rgba(255,255,255,0.06)' }}>
            <h2 className="text-2xl font-bold text-white mb-1">Create account</h2>
            <p className="text-white/40 text-sm mb-6">Join Malaysia Co (Maintenance Services) - it is free</p>
            ...
            {tab === 'password' && (
              <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="space-y-4">
                <Field label="Full Name" error={passwordForm.formState.errors.name?.message}>
                  <AuthInput type="text" placeholder="John Doe" autoComplete="name" {...passwordForm.register('name')} />
                </Field>
                ...
                <Field label="I am a">
                  <RoleToggle value={passwordForm.watch('role')} onChange={(v) => passwordForm.setValue('role', v)} />
                </Field>
                <Field label="Password" error={passwordForm.formState.errors.password?.message}>
                  <AuthInput type="password" placeholder="Min 8 characters" autoComplete="new-password" {...passwordForm.register('password')} />
                </Field>
                <Field label="Confirm Password" error={passwordForm.formState.errors.confirm?.message}>
                  <AuthInput type="password" placeholder="Repeat password" autoComplete="new-password" {...passwordForm.register('confirm')} />
                </Field>
            ...
            {tab === 'magic' && (
              hasSentLink ? (
                ...
                <form onSubmit={magicForm.handleSubmit(onMagicSubmit)} className="space-y-4">
                  <Field label="Full Name" error={magicForm.formState.errors.name?.message}>
                    <AuthInput type="text" placeholder="John Doe" autoComplete="name" {...magicForm.register('name')} />
                  </Field>
                  ...
                  <Field label="I am a">
                    <RoleToggle value={magicForm.watch('role')} onChange={(v) => magicForm.setValue('role', v)} />
                  </Field>
                  <GradientButton type="submit" disabled={isLoading}>
            ...
            <p className="text-center text-sm text-white/30 mt-6">
              ...
              <Link href="/login" className="text-[#00d4ff] hover:text-[#00ffa3] transition-colors font-medium">
                Sign in
              </Link>
            </p>
          </div>
...
function RoleToggle({ value, onChange }: { value: string; onChange: (v: 'customer' | 'technician') => void }) {
  return (
    <div className="flex gap-2">
      {(['customer', 'technician'] as const).map((r) => (
        <button
          ...
          style={value === r
            ? { background: 'linear-gradient(135deg, rgba(0,212,255,0.2), rgba(0,255,163,0.2))', color: '#fff', border: '1px solid rgba(0,212,255,0.4)' }
